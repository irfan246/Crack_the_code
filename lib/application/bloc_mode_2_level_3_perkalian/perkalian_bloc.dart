...
class PerkalianBlocMode2Level3 extends Bloc<PerkalianEvent, PerkalianState> {
  static const int _timeLimit = 60;
  late Timer _timer;
  int _remainingTime = _timeLimit;

  PerkalianBlocMode2Level3()
      : super(PerkalianLoadedState(safes: _generateInitialSafes())) {
    on<PerkalianMode2Level3StartTimer>(_onStartTimer);
    on<SelectNumber>(_onSelectNumber);
    on<CheckWin>(_onCheckWin);
    on<PerkalianMode2Level3ResetEvent>(_onRestartGame);
    on<UpdateTime>(_onUpdateTime);
    on<TimeOut>(_onTimeOut);
  }

  void _onStartTimer(
      PerkalianMode2Level3StartTimer event, Emitter<PerkalianState> emit) {
  ...
  void _onRestartGame(
      PerkalianMode2Level3ResetEvent event, Emitter<PerkalianState> emit) {
    _timer.cancel();
    _remainingTime = _timeLimit;
    add(PerkalianMode2Level3StartTimer());
